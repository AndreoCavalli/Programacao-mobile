...
class Jogador {
    // Método para receber cartas e analisar suas combinações
    fun receberCartas(cartas: List<Carta>) {
        // Imprime as cartas recebidas
    ...
    // Método privado para verificar se há um flush
    private fun isFlush(cartas: List<Carta>): Boolean = cartas.map { it.naipe }.toSet().size == 1
    // Método privado para verificar se há um Full House
